...
for img in scans:
    ...
    conf_text = []
    for i in range(0, len(text['text'])):
        # print(text['conf'][i] , text['text'][i])
        if text['conf'][i] > 30:
            conf_text +=text['text'][i]
...
# 문장에 띄어쓰기 추가
sentence = ''.join(conf_text)
spacing = Spacing()
kospacing_result = spacing(sentence)

print("Original Sentence:", sentence)
print("Spaced Sentence:", kospacing_result)


# TensorFlow BERT 모델을 로드하기 위해 tokenizer와 model을 사용합니다.
...
# 위의 tokenizer와 model을 사용하여 FillMaskPipeline을 생성합니다.
pip = FillMaskPipeline(model=model, tokenizer=tokenizer)

# 마스킹된 문장의 단어를 예측합니다.
result = pip( kospacing_result)
print(result)
